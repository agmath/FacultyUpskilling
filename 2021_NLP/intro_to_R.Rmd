---
title: 'An Introduction to `tidyverse` in R'
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
library(tidyverse)
#setwd("/Users/williamjamieson/Desktop/Data Science/MAT300/Day to Day") 
```
...
Let's arrange the rust here by mean highway mile per gallon rating in the default ascending order.
...
dataset %>%
  mutate("new column name" = "function of old columns")
```
...
A common trick is to use the `if_else()` function to create a categorical variable from a numerical one. The three arguments of `if_else()` are a condition, and the values you want to fill if the condition is true or false, respectively.
...
There is a lot more to learn about tidying data (especially about using regular expressions), but this document should provide you with a solid start. I hope that you see from these examples why tidying data is important for effective visualization and data exploration!
